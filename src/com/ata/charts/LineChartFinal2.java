...
public class LineChartFinal2 {
    ...
    public LineChartFinal2(PApplet p) {
        this.p = p;
        applyLightThemeColors();
        palette = defaultLightPalette();
    }

    public LineChartFinal2 setTitle(String t) { this.title = (t == null) ? "" : t; return this; }
    public LineChartFinal2 setXLabel(String t) { this.xLabel = (t == null) ? "" : t; return this; }
    public LineChartFinal2 setYLabel(String t) { this.yLabel = (t == null) ? "" : t; return this; }
    public LineChartFinal2 setLegendInteractive(boolean enabled) { this.legendInteractive = enabled; return this; }
    public LineChartFinal2 setAnimationEnabled(boolean enabled) { this.animationEnabled = enabled; return this; }
    public LineChartFinal2 setAnimationDurationMs(int ms) { if (ms > 0) this.animationDurationMs = ms; return this; }
    public LineChartFinal2 setPreserveCustomPaletteAcrossModeChange(boolean preserve) { this.preserveCustomPaletteAcrossModeChange = preserve; return this; }
    public LineChartFinal2 setReferenceLine(float value, String label) { this.referenceLineValue = value; this.referenceLineLabel = (label == null) ? "" : label; return this; }
    public LineChartFinal2 setPageSize(int n) { if (n > 0) this.pageSize = n; clampCurrentPage(); rebuildPlotData(); return this; }

    public LineChartFinal2 setAxisInterpretation(AxisInterpretation interpretation) {
    ...
    public LineChartFinal2 setTranspose(boolean transpose) {
        return setAxisInterpretation(transpose ? AxisInterpretation.SERIES_ON_X : AxisInterpretation.CATEGORIES_ON_X);
    }
    ...
    public LineChartFinal2 toggleAxisInterpretation() {
        clearPointBuffers();
        selectedSeries = -1;
        selectedIndex = -1;
        axisInterpretation = isTransposed() ? AxisInterpretation.CATEGORIES_ON_X : AxisInterpretation.SERIES_ON_X;
        rebuildPlotData();
        clearPointBuffers();
        restartAnimation();
        return this;
    ...
    public LineChartFinal2 enableDefaultAnalyticalUI() {
        helpVisible = true;
        statusVisible = true;
        detailsVisible = true;
        gridVisible = true;
    ...
    public LineChartFinal2 restartAnimation() {
        animationStartMs = p.millis();
        return this;
    }

    public LineChartFinal2 setDataFromTable(Table t, String categoryCol, String[] valueCols) {
    ...
    public LineChartFinal2 setPalette(int[] colors) {
    ...
    public LineChartFinal2 setPaletteByName(String paletteName) {
    ...
    public LineChartFinal2 setSeriesColor(String seriesName, int color) {
    ...
    public LineChartFinal2 setCustomSeriesColors(String[] seriesNames, int[] colors) {
        ...
        for (int i = 0; i < n; i++) pendingNamedSeriesColors.put(seriesNames[i], colors[i]);
        applyPendingNamedColors();
        return this;
    }

    public LineChartFinal2 toggleCustomColors() {
        if (plotSeriesNames == null) return this;
        if (customColorsActive) {
            customColorsActive = false;
            if (savedPaletteBeforeCustom != null) palette = Arrays.copyOf(savedPaletteBeforeCustom, savedPaletteBeforeCustom.length);
            ensurePaletteCapacity();
            applyPendingNamedColors();
        } else {
    ...
    public LineChartFinal2 toggleMode() {
        if (darkMode) setLightMode(); else setDarkMode();
        return this;
    }

    public LineChartFinal2 setLightMode() {
        darkMode = false;
        applyLightThemeColors();
    ...
    public LineChartFinal2 setDarkMode() {
        darkMode = true;
    ...
    public void handleKeyPressed(char key, int keyCode) {
        if (key == 'h' || key == 'H') helpVisible = !helpVisible;
            ...
            if (currentPage > 0) { currentPage--; rebuildPlotData(); restartAnimation(); }
        } else if (keyCode == PApplet.RIGHT) {
            if (canGoToNextPage()) { currentPage++; rebuildPlotData(); restartAnimation(); }
        }
    }
    ...
    public void draw(int x, int y, int w, int h) {
        ...
        if (plotW <= 10 || plotH <= 10) { p.popStyle(); return; }
        ...
        float anim = animationEnabled ? PApplet.constrain((p.millis() - animationStartMs) / (float) animationDurationMs, 0, 1) : 1;
        ...
        for (int s = 0; s < plotValues.length; s++) {
            ...
            for (int i = 0; i < plotXLabels.length; i++) {
                float v = plotValues[s][i];
                if (Float.isNaN(v)) continue;
                float px = (plotXLabels.length == 1) ? plotX + plotW * 0.5f : plotX + (i / (float) visibleCount) * plotW;
            ...
            for (int i = 0; i < plotXLabels.length; i++) {
                ...
                p.noStroke();
                p.fill(fillColor);
                float r = pointRadius + ((hoveredSeries == s && hoveredIndex == i) ? 2.5f : 0f) + ((selectedSeries == s && selectedIndex == i) ? 1.5f : 0f);
        ...
        if (hoveredSeries >= 0 && hoveredIndex >= 0) drawTooltip();
    ...
    private void drawGrid(float plotX, float plotY, float plotW, float plotH, NiceAxis axis) {
        ...
        for (int i = 0; i < count; i++) {
            float gx = (count == 1) ? plotX + plotW * 0.5f : plotX + (i / (float) Math.max(1, count - 1)) * plotW;
    ...
    private void drawReferenceLine(float plotX, float plotY, float plotW, float plotH, float yMin, float yMax) {
        ...
        p.textAlign(PApplet.LEFT, PApplet.BOTTOM);
        String label = referenceLineLabel.length() == 0 ? "Ref " + formatNumber(referenceLineValue) : referenceLineLabel + " " + formatNumber(referenceLineValue);
    ...
    private void resetView() {
        visibilityBySeriesName.clear();
        clearPointBuffers();
        selectedSeries = selectedIndex = -1;
        currentPage = 0;
        rebuildPlotData();
        restartAnimation();
    }
    ...
    private void rebuildPlotData() {
        ...
        if (axisInterpretation == AxisInterpretation.CATEGORIES_ON_X) {
            ...
            if (pageSize < Integer.MAX_VALUE) {
                ...
                if (start >= total) { currentPage = 0; start = 0; end = Math.min(total, pageSize); }
            }
    ...
    private boolean canGoToNextPage() { return currentPage + 1 < totalPages(); }
    ...
    private int seriesIndex(String name) {
        ...
        for (int i = 0; i < plotSeriesNames.length; i++) if (name.equals(plotSeriesNames[i])) return i;
        return -1;
    }

    private int[] defaultLightPalette() {
        return new int[] { 0xff1f77b4, 0xffff7f0e, 0xff2ca02c, 0xffd62728, 0xff9467bd, 0xff8c564b, 0xffe377c2, 0xff7f7f7f };
    }

    private int[] defaultDarkPalette() {
        return new int[] { 0xff4db6ff, 0xffff8a65, 0xff81c784, 0xfff06292, 0xffba68c8, 0xffffd54f, 0xff90a4ae, 0xffaed581 };
    }
